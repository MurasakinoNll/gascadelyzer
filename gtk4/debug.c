...
static gboolean refresh_durations(gpointer user_data) {
    ...
    // Free any previous start times before parsing new log data
    while (data->cleansing_start_times) {
        gdouble *start_time_ptr = (gdouble *)data->cleansing_start_times->data;
        g_free(start_time_ptr);  // Free the dynamically allocated double
    ...
    while (fgets(line, sizeof(line), file)) {
        if (strstr(line, "Cleansing SurvivalLifeSupportPillarCorruptible")) {
            ...
            *start_time_ptr = timestamp;
            ...
            if (data->cleansing_start_times != NULL && g_list_length(data->cleansing_start_times) > 0) {
                ...
                // Store a dynamically allocated duration
                gdouble *duration_ptr = g_new(gdouble, 1);
                *duration_ptr = duration;
                durations = g_list_append(durations, duration_ptr);

                g_free(start_time_ptr);  // Free the start time
                data->cleansing_start_times = g_list_delete_link(data->cleansing_start_times, data->cleansing_start_times);
        ...
        if (cleanse_count >= 3) {
            ...
            // Free the durations list elements after use

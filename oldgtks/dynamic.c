...
#ifdef _WIN32
#define DEFAULT_PATH "%APPDATA%\\local\\Warframe\\EE.log"
#else
#ifdef DEV_MODE
#define DEFAULT_PATH "/home/haraku/harakdev/gascade/flyra.filtered"
#else
#define LINUX_ID "230410"  // replace with pathID
...
static gboolean refresh_count(gpointer user_data) {
    ...
    if (home != NULL) {
        snprintf(expanded_path, MAXLEN, "%s/.local/share/Steam/steamapps/compatdata/230410/pfx/drive_c/users/steamuser/AppData/Local/Warframe/EE.log", home);
    } else {
        g_print("Error: HOME environment variable is not set.\n");
